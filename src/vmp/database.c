...
void db_add_entries(vec_t* entries);
void db_add_entry(char* path, map_t* entry);
void db_remove_entry(map_t* entry);
void db_remove_non_existing(map_t* files);
void db_filter_existing(map_t* files);
...
map_t*   db_get_db();
void     db_get_entries(vec_t* entries);
uint32_t db_count();
void     db_reset();
...
void db_add_entries(vec_t* entries)
{
	...
	char*  path  = MGET(entry, "path");
	if (path) db_add_entry(path, entry);
    }
}
